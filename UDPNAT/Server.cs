...
namespace UDPNAT
{
    ...
    public class Server
    {
        ...
        {
            byte[] msgBuffer;
            IPEndPoint _remoteFileAckPoint = null;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    else if (msgType == typeof(C2S_FileBlockAckMessage))
                    {
                        var msgAckFromClient = (C2S_FileBlockAckMessage)msgObject;
                        var msgAckToAnotherClient = new S2C_FileBlockAckMessage(msgAckFromClient.CheckSum);
                        //DoWriteLog("FileBlockAck: " + msgAckFromClient.ToUserIPEndPoint);
                        if(_remoteFileAckPoint == null)
                        {
                            return;
                        }
                        SendMessage(msgAckToAnotherClient, _remoteFileAckPoint);
                    }
                    else if(msgType == typeof(C2S_ReadytoSendFileMessage))
                    {
                        var ackToSenderClient = new S2C_AckReadytoSendFileMessage();
                        _remoteFileAckPoint = _remotePoint;
                        SendMessage(ackToSenderClient, _remotePoint); //收到客戶端準備發送文件的請求，在予以echo返回。建立C-S連接
                    }
                }
